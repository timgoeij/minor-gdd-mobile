...
public class PlayerScript : ColorChangeableObject, IHitable {
    ...
    public GameObject firstColorTarget
    {
        get;
        set;
    }
	...
	void FixedUpdate () {
		...
		if (_nearDeath != null) {
            ...
            if(!_isDead)
            {
                ...
                {
                    tm.PlayerIsAtPostion();
                    tutorialEventAlreadyActivated = false;
                }
                else
                {
                    tm.IsAtPosition = false;
                }
            }
		}
		else
    ...
    private void CheckTutorialDistance()
    {
        ...
        {
            tm.PlayerIsNotInPosition();
            firstColorTarget = null;
            return;
        }
            

        RaycastHit2D tutorialHit = Physics2D.Raycast(transform.position, Vector2.right, CameraScreen.width * .8f);

        if (tutorialHit.collider != null && tutorialHit.collider.GetComponent<Laser>() != null && !tutorialEventAlreadyActivated)
        {
            tm.PlayerIsAtPostion();
            firstColorTarget = tutorialHit.collider.gameObject;
	...
	private void CheckNearDeath() {
		RaycastHit2D[] hit = Physics2D.CircleCastAll( transform.position, 1f, (speed > 0) ? Vector2.right : Vector2.left, 3f);
